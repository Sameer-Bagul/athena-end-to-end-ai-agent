...
app.whenReady().then(async () => {
    ...
    globalShortcut.register('Alt+Space', () => {
        ...
        if (main && !main.isDestroyed())
            main.webContents.send('shortcut:pressed');
        if (widget && !widget.isDestroyed())
            widget.webContents.send('shortcut:pressed');
    });
    ...
    mcpManager.onStatus = (name, status) => {
        ...
        const widget = getWidgetWindow();
        if (main && !main.isDestroyed())
            main.webContents.send("agent:mcp-status", { name, status });
        if (widget && !widget.isDestroyed())
            widget.webContents.send("agent:mcp-status", { name, status });
    };
    // Spawn PoC sidecars
    ...
    mcpManager.spawnServer("agent-browser", "npx", ["-y", "agent-browser@latest", "mcp", "--tools", "all", "--session", "athena_vault"], app.getAppPath());
});
app.on('will-quit', () => {
