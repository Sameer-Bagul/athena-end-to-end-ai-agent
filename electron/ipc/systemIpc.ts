...
export function registerSystemIpcHandlers() {
  ...
  ipcMain.on("agent:add-timer", (_, { duration, unit, label }) => {
    const main = getMainWindow();
    const widget = getWidgetWindow();
    if (main && !main.isDestroyed()) main.webContents.send("athena:add-timer-ipc", { duration, unit, label });
    if (widget && !widget.isDestroyed()) widget.webContents.send("athena:add-timer-ipc", { duration, unit, label });
  });

  ipcMain.on("agent:remove-timer", (_, { id }) => {
    const main = getMainWindow();
    const widget = getWidgetWindow();
    if (main && !main.isDestroyed()) main.webContents.send("athena:remove-timer-ipc", { id });
    if (widget && !widget.isDestroyed()) widget.webContents.send("athena:remove-timer-ipc", { id });
  });
  ...
  ipcMain.handle("system:set-volume", async (_, percent: number) => systemService.setVolume(percent));
  ipcMain.handle("system:set-brightness", async (_, percent: number) => systemService.setBrightness(percent));
  ipcMain.handle("system:get-battery", async () => systemService.getBatteryInfo());
  ipcMain.handle("system:list-files", async (_, targetPath: string) => systemService.listFiles(targetPath));
  ipcMain.handle("system:read-file", async (_, targetPath: string) => systemService.readFile(targetPath));
  ipcMain.handle("system:file-stats", async (_, targetPath: string) => systemService.getFileStats(targetPath));
