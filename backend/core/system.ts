...
export const systemService = {
     ...
     * Get system volume
     */
    async getVolume(): Promise<number | string> {
        try {
     ...
     */
    async setBrightness(percent: number): Promise<string> {
        try {
            const val = Math.max(0, Math.min(100, percent));
     ...
     */
    async listFiles(targetPath: string): Promise<any[]> {
        try {
     ...
     */
    async getFileStats(targetPath: string): Promise<any> {
        try {
            const resolvedPath = targetPath.startsWith('~')
                ? path.join(homedir(), targetPath.slice(1))
                : path.resolve(targetPath);
